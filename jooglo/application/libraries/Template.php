...
class Template
{
	...
	function initialize($config = array())
	{
		...
		{
			// Let's use this obvious default
			$this->_asset_locations = array('jooglo/themes/');
		}
	...
	public function get_layouts()
	{
		...
		foreach(glob(self::_find_view_folder().$_layout_folder.'*.*') as $layout)
		{
	...
	public function get_theme_layouts($theme = NULL)
	{
		...
		foreach ($this->_theme_locations as $location)
		{
			...
			if( is_dir($location.$theme.'/views/web/'.$_layout_folder) )
			{
				foreach(glob($location.$theme . '/views/web/'.$_layout_folder.'*.*') as $layout)
				{
					$layouts[] = pathinfo($layout, PATHINFO_BASENAME);
				}
				break;
			...
			if(is_dir($location.$theme.'/views/'.$_layout_folder))
			{
				foreach(glob($location.$theme . '/views/'.$_layout_folder.'*.*') as $layout)
				{
					$layouts[] = pathinfo($layout, PATHINFO_BASENAME);
				}
				break;
	...
	public function layout_exists($layout)
	{
		...
		return file_exists(self::_find_view_folder().$_layout_folder . $layout . self::_ext($layout));
	}
